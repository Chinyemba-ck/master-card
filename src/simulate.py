"""
simulate.py — HealthScore what-if scenarios for Winnsboro, LA (Franklin Parish).

═══════════════════════════════════════════════════════════════════════════
EVIDENCE BASE — Two complementary analyses ground every number below
═══════════════════════════════════════════════════════════════════════════

1. WINNSBORO'S OWN HISTORY (2017-2025)
   What Winnsboro's indicators looked like when it scored 42 (its best):
   ...
   Winnsboro has BEEN at these levels. Phase 1-2 targets are restorations,
   not aspirations.

   Winnsboro year-on-year IGS moves, what drove them:
   ...
   KEY LESSON: In Winnsboro's data, Small Biz Loans and Early Education
   are the proven IGS levers. CommDiv was never high in the 42-IGS years.

2. ARCHIBALD CORRELATION ANALYSIS (what drives Archibald's IGS 59)
   Pearson r with Archibald IGS across 9 years:
     ...
     Small Biz Loans     r=-0.231  <- NEGATIVE (Archibald's loans fell as IGS rose)
     Internet Access     r=-0.379  <- NEGATIVE

   What Archibald locked in at 2025 (sustained at all-time highs):
     Early Education:  69  | Labor Engagement: 58 | Female Poverty: 65
     Net Occupancy:    85  | Personal Income:  56 | Travel Time:    70
   These 6 indicators explain Archibald staying at IGS 58-59.

   Archibald's 2023 breakout (+9 in one year):
   ...
   CRITICAL INSIGHT: Small Biz Loans and CommDiv do NOT drive Archibald's
...
WINNSBORO HISTORY CORRELATION WITH IGS (r across 9 years):
  ...
  NOTE: In Winnsboro, CommDiv has r=-0.426 with IGS (negative! — CommDiv
  was highest in 2024 when IGS was 35). Do not claim CommDiv as primary
  lever in Winnsboro-specific framing.
...
RIDGE COEFF (national model, 757,582 obs, R²=0.937):
  ...
  is the top Ridge driver, matching Archibald r=+0.943.
  Spend Growth is the new 18th indicator (weak, +0.56); Winnsboro has
...
Small Business Loans [current=44, Winnsboro r=+0.720, Ridge coeff=+1.25]
  Phase 1: 44 -> 55
    Winnsboro history: was 72-74 in 2017-18. 55 is partial restoration.
    ...
    PROOF: Winnsboro at 30 (2019) recovered to 47 in one year, 60 in 2023.
    +11 pts is well within Winnsboro's demonstrated recovery range.
  Phase 2: 55 -> 62
    Lender familiarity with HealthScore scores reduces friction.
    Winnsboro had 60 in 2023 already — this is not new ground.
  Phase 3: 62 -> 66
    66 = empirical capital floor for all 3 IGS 60+ tracts in dataset.
    Archibald sits at 50 (not the loan driver for IGS) — we target 66
    because that is what Fresno+Dallas have, not to match Archibald.

Commercial Diversity [current=36, Winnsboro r=-0.426, Ridge coeff=+2.81]
  Phase 1: 36 -> 42
    REVISED DOWN from 50. Winnsboro correlation with IGS is NEGATIVE
    (-0.426) — CommDiv was highest (62) in 2024 when IGS was 35.
    Archibald's CommDiv is only 21 (lower than Winnsboro!) and IGS=59.
  ...
  Phase 2: 42 -> 52
    ...
    Winnsboro's own data this is NOT the primary lever.
  Phase 3: 52 -> 65
...
Min/Women Owned Businesses [current=15, Ridge coeff=+0.27]
  ...
  Phase 3: 45 -> 60
    5-year horizon. Archibald=82 is long-term benchmark; 60 is midway.
    Low Ridge coeff (+0.27) — this contributes less to IGS directly.

Labor Market Engagement [current=14, Winnsboro r=+0.729, Ridge coeff=+1.44]
  Phase 1: HELD at 14
    1-2 year lag before childcare -> women work chain appears in data.
    Winnsboro was at 48 in 2017 — we know this is reversible.
  Phase 2: 14 -> 28
    REVISED UP from 20. Archibald proof: Labor Engagement r=+0.911 with
    its IGS. Winnsboro's own history: was 48 in 2017. Childcare expansion
    (Early Ed 19->32) enables women to re-enter workforce. 28 is still
    well below Winnsboro's 2017 level of 48.
  Phase 3: 28 -> 42
    REVISED UP from 35. Remote RHTP healthcare admin/billing jobs add to
    labor base. 42 is close to Winnsboro's 2017 level (48) and approaching
    Archibald 2025 (58). This is a PRIMARY IGS lever — Archibald r=+0.91.

New Businesses [current=73, Winnsboro r=+0.802, Ridge coeff=+1.10]
  All phases: HELD at 73
    Winnsboro already ABOVE Archibald (68) and 60+ avg (64). This is a
    STRENGTH. Note: Winnsboro's r=+0.802 — when New Businesses was high
...
Early Education [current=19, Winnsboro r=+0.776, Ridge coeff=+1.13]
  Phase 1: 19 -> 30
    REVISED UP from 28. Winnsboro was at 78 (2017), 55 (2018), 65 (2020).
    The current 19 is a constructed collapse, not a structural limit.
    Archibald Proof: jumped +22 in one year (2023) when childcare invested.
    We claim +11 for Year 1 — half of what Archibald achieved in one year.
    Mechanism: HealthScore stabilizes childcare centers financially ->
    they stay open, add slots, attract enrollment.
  Phase 2: 30 -> 42
    REVISED UP from 35. Winnsboro's own 2020 level was 65. Archibald
    r=+0.71 — this feeds directly into labor and income chain.
    More financially stable centers = more open slots. 42 is still well
    below Winnsboro's 2020 level (65) and Archibald 2025 (69).
  Phase 3: 42 -> 55
    REVISED UP from 39. Archibald is at 69; 55 is a realistic 5-year
    target given Winnsboro's proven 65 in 2020. This is RESTORATION.

Personal Income [current=36, Archibald r=+0.943, Ridge coeff=+0.83]
  Phase 1: HELD at 36
    Income follows employment; too early for Year 1.
  Phase 2: 36 -> 43
    REVISED UP from 40. Archibald r=+0.943 — strongest single Archibald
    driver. Healthcare staff at stabilized providers earn wages.
    Archibald jumped Personal Income +16 in 2023 when labor chain fired.
    Conservative +7 for Year 2-3.
  Phase 3: 43 -> 52
    REVISED UP from 48. As labor engagement reaches 42, more residents
    earn wages. Archibald=56; 52 is close. This is a PRIMARY indicator
    for reaching Archibald-level IGS — r=0.94 in Archibald's data.

Female Above Poverty [current=69, Archibald r=+0.901, Ridge coeff=+2.09]
  Phase 1: HELD at 69
    Outcome of childcare chain; 1-2 year lag.
  Phase 2: 69 -> 75
    Childcare access allows women to work. Archibald jumped +32 in 2023.
    Conservative +6 for Year 2-3. Winnsboro already beats Archibald (65).
    High Ridge coeff (+2.09) — meaningful IGS impact from small gain.
  Phase 3: 75 -> 80
    Building on existing strength. 80 is conservative given Archibald
    r=+0.901 and Winnsboro's starting advantage.
...
Health Insurance [current=59, Ridge coeff=-1.41]
  All phases: HELD at 59
    NEGATIVE Ridge coefficient. Winnsboro=93.5% coverage — do not target.

───────────────────────────────────────────────────────────────────────────
PLACE PILLAR
───────────────────────────────────────────────────────────────────────────

Internet Access [current=2, Archibald r=-0.379, Ridge coeff=-0.73]
  Phase 1: HELD at 2
    NELPCO/Volt $54M fiber — not HealthScore. ALSO: in Archibald's data,
    Internet Access has NEGATIVE correlation with IGS (-0.379). Archibald
...
Real Estate Value [current=46, Archibald r=+0.683, Ridge coeff=+2.53]
  ...
  Phase 3: 46 -> 58
    ...
    Archibald r=+0.683 — moderate relationship. +12 pts in Years 4-5.
    High Ridge coeff (+2.53) makes even a small move meaningful.

Travel Time to Work [current=9, Archibald r=+0.852, Ridge coeff=+0.76]
  Phase 1: HELD at 9
  Phase 2: 9 -> 18
    REVISED UP from held. Archibald r=+0.852 — 4th strongest Archibald
    driver. Remote RHTP healthcare admin/billing jobs begin to reduce
    commute burden. Archibald went from 24 (2021) to 70 (2023) in 2 years.
    Cautious +9 for Year 2-3 given Winnsboro's structural geography gap.
  Phase 3: 18 -> 28
    Remote-work adoption grows. Note: Travel Time r=+0.628 in Winnsboro
    too. This is a real lever — Archibald's clearest evidence.
    Archibald=70; 28 is very conservative given the Archibald trajectory.

Affordable Housing [current=56, Ridge coeff=+0.40]
  All phases: HELD at 56
    Above Archibald; not a priority gap. Low coeff.
...
FINAL SCENARIO TABLE — revised numbers with dual evidence base
  ...
  W-corr = Winnsboro 9yr correlation | A-corr = Archibald 9yr correlation
═══════════════════════════════════════════════════════════════════════════
  Indicator            Cur  Ph1  Ph2  Ph3  Type  W-corr  A-corr  Key rationale
  Small Biz Loans       44   55   62   66    D    +0.72   -0.23   Winnsboro restoration (was 74); SELAHEC
  Early Education       19   30   42   55    D    +0.78   +0.71   Winnsboro restoration (was 78/65); childcare
  Labor Mkt Eng         14   14   28   42    I    +0.73   +0.91   Archibald's #2 driver; childcare chain lag
  Personal Income       36   36   43   52    I    -0.66   +0.94   Archibald's #1 driver; follows labor chain
  Female Above Pov      69   69   75   80    I    -0.71   +0.90   Archibald's #3 driver; Winnsboro already strong
  Net Occupancy         38   38   42   52    I    +0.24   +0.83   Archibald's #5 driver; jobs->people stay
  Travel Time to Work    9    9   18   28    I    +0.63   +0.85   Archibald's #4 driver; remote RHTP jobs
  ...
  Comm Diversity        36   42   52   65    D    -0.43   +0.42   NOT a Winnsboro IGS driver; modest recovery
  Min/Women Biz         15   25   45   60    D    -0.99   -0.54   New ground; Ridge +1.35 (national)
  Spend Growth         NaN  med  med  med    X    N/A     N/A     New 18th indicator; Winnsboro has no history; hold at median
  Internet Access        2    2   35   70    X    +0.69   -0.38   NELPCO/Volt infra; Archibald fell as IGS rose
...
def run_simulations(ridge, rf, gb, scaler, baseline, franklin_vec, feature_names, df_model, actual_igs):
    ...
    print("=" * 55)
    print("WINNSBORO, LA (FRANKLIN PARISH) -- WHAT-IF SIMULATION")
    print("=" * 55)
